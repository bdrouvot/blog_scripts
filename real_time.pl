...
# V2.0 (2013/07): For asmiostat: Show database instance and sort_field on READS|WRITES
# V2.1 (2013/09): For asmiostat: Can sort on READS+WRITES=IOPS
# V2.2 (2013/10): For asmiostat: deprecated, download asm_metrics.pl instead
#
# Description:
...
sub report_values {
	...
	foreach my $pk (sort {$diffsnaps{$a}[0] <=> $diffsnaps{$b}[0] || $diffsnaps{$b}[$sort_fields{uc($sort_field_pattern)}] <=> $diffsnaps{$a}[$sort_fields{uc($sort_field_pattern)}] || $diffsnaps{$a}[1] cmp $diffsnaps{$b}[1] || $diffsnaps{$a}[2] cmp $diffsnaps{$b}[2] || $diffsnaps{$a}[4] cmp $diffsnaps{$b}[4] || $diffsnaps{$a}[11] cmp $diffsnaps{$b}[11]} (keys(%diffsnaps))) {
	&report_resultset($pk,%diffsnaps);
	debug("Sorted value is : ".$diffsnaps{$pk}[$sort_fields{uc($sort_field_pattern)}]);
	}
	} else {
	foreach my $pk (sort {$diffsnaps{$a}[0] <=> $diffsnaps{$b}[0] || $diffsnaps{$b}[$sort_fields{uc($sort_field_pattern)}] <=> $diffsnaps{$a}[$sort_fields{uc($sort_field_pattern)}] || $diffsnaps{$a}[1] cmp $diffsnaps{$b}[1] || $diffsnaps{$a}[3] cmp $diffsnaps{$b}[3] ||  $diffsnaps{$a}[12] cmp $diffsnaps{$b}[12]} (keys(%diffsnaps))) {
	&report_resultset($pk,%diffsnaps);
	debug("Sorted value is : ".$diffsnaps{$pk}[$sort_fields{uc($sort_field_pattern)}]);
	}
	}
...
printf ("  %-15s   %-30s %-100s \n",'','asmiostat','This option is deprecated: download asm_metrics.pl instead');
print ("\n");
...
print "\n";
print "This option is deprecated: download asm_metrics.pl instead\n";
print "see: http://bdrouvot.wordpress.com/asm_metrics_script/\n";
print "\n";
exit 1;
}
