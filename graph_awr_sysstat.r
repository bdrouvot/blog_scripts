...
#
# Description:
# Utility used to display graphically for a system statistic
#
#			- Value for the stat
...
#
# Output : <sysstat>.txt : The result in txt format
#          <sysstat>.pdf : The graph saved in pdf format
#
#
...
# My query
myquery<-"
select to_char(s.begin_interval_time,'YYYY/MM/DD HH24:MI') as dateevt,
sta.stat_name as stat_name,
sta.VALUE as VALUE,
round(((sta.VALUE)/
(
...
value - first_value(value) over (partition by stat_name order by snap_id rows 1 preceding) as VALUE
from
dba_hist_sysstat
where stat_name='"
...
#
sqstat<-dbGetQuery(conn,myquery)
#

# Save the query output to a file
file_stat_name<-gsub(" ", "_", stat_name,fixed=TRUE)
capture.output(sqstat,file=paste(file_stat_name,".txt",sep=""), append=FALSE)
...
sqstat$DATEEVT<-strptime(sqstat$DATEEVT,"%Y/%m/%d %H:%M")
...
skip<-length(sqstat$DATEEVT)/40
plot(sqstat[,'DATEEVT'],sqstat[,'VALUE'],type="l",col="blue",xaxt="n",main=stat_name,cex.main=2,xlab="",ylab="VALUE")
Axis(side=1,at=sqstat$DATEEVT,round(skip),format="%Y/%m/%d %H:%M")
plot(sqstat[,'DATEEVT'],sqstat[,'VALUE_PER_SEC'],type="l",col="blue",xaxt="n",xlab="",ylab="VALUE_PER_SEC")
Axis(side=1,at=sqstat$DATEEVT,round(skip),format="%Y/%m/%d %H:%M")
hist(sqstat[,'VALUE'],xlab="VALUE",main=NULL,border="blue")
hist(sqstat[,'VALUE_PER_SEC'],xlab="VALUE_PER_SEC",main=NULL,border="blue")
